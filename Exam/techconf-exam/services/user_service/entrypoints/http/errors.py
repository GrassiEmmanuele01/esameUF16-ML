"""Mapping degli errori verso le risposte HTTP.

Traduce le eccezioni di dominio (sottoclassi di :class:`common.AppError`) e i
body JSON malformati nella struttura d'errore uniforme di piattaforma:
...
def register_error_handlers(app: Flask) -> None:
    ...
    def _handle_bad_request(exc: HTTPException) -> Response:
        # Un 400 di Flask sul parsing del body indica JSON malformato.
    ...
    def _handle_method_not_allowed(exc: HTTPException) -> Response:
        return _error_response(
            "METHOD_NOT_ALLOWED", "Metodo non consentito", 405
        )

    @app.errorhandler(Exception)
    def _handle_unexpected(exc: Exception) -> Response:
        # Non intercettare le HTTPException già gestite sopra.
