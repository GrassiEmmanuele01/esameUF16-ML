...
- [x] 4. Validazione dei payload (regole di schema e date)
...
- [x] 5. Casi d'uso EventService (logica di dominio)
...
- [x] 7. Client HTTP verso user-service
...
- [x] 8. HTTP: serializzazione, mapping errori e rotte
...
- [x] 9. Wiring finale (composition root)
...
- [x] 10. Unit test del dominio (EventService)
...
- [x] 11. Unit test dei backend di persistenza (parametrizzati)
  - Suite parametrizzata sui tre backend con `tmp_path`: CRUD, filtri `status`/`city`, paginazione, persistenza.
  - _Requirements: 5.x_

- [x] 12. Unit test del client user-service (responses)
...
- [x] 13. Unit test HTTP + validazione contratto
...
- [x] 14. Accettazione (opzionale, richiede services.yaml)
